...
class FedEraser(Server):
    def __init__(self, args, times):
        ...
        self.total_unlearning_time = 0  # 新增：用于记录总恢复时间
    ...
    def unlearning(self):
        unlearning_start_time = time.time()  # 记录恢复开始时间
        ...
        for epoch in range(0, self.global_rounds):
            ...
            for client in self.remaining_clients:
                ...
                client.train()
            self.new_CM = copy.copy(self.remaining_clients)
        ...
        unlearning_end_time = time.time()  # 记录恢复结束时间
        self.total_unlearning_time = unlearning_end_time - unlearning_start_time  # 计算总恢复时间
        print(f"Total unlearning time: {self.total_unlearning_time} seconds")  # 打印总恢复时间
