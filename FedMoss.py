...
from eraser_jy_zx import eraser_jy_zx
from eraser_jy import eraser_jy
from recover_jy import recover_jy
from recover_jy_zx import recover_jy_zx
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        elif args.algorithm == "eraser_jy":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = eraser_jy(args, i)
        ...
        elif args.algorithm == "eraser_jy_zx":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = eraser_jy_zx(args, i)
        ...
        elif args.algorithm == "recover_jy":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = recover_jy(args, i)
        ...
        elif args.algorithm == "recover_jy_zx":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = recover_jy_zx(args, i)
        ...
        elif args.algorithm == "crab_jy":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = crab_jy(args, i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-nb', "--num_classes", type=int, default=25)
    ...
    parser.add_argument('-robust', "--robust_aggregation_schemes", type=str, default="FedAvg",
                        choices=["TrimmedMean", "Median", "Krum"],
                        help="The aggregation schemes using when calculating the server parameters")
