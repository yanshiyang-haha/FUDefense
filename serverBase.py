...
from sklearn.metrics import precision_score, recall_score, f1_score  # 导入新指标计算函数
...
class Server(object):
    def __init__(self, args, times):
        ...
        self.select_round_ratio = 0.8
        self.select_client_ratio = 0.8
    ...
    def receive_models(self):
        if self.selected_clients == []:
            print("Warning: self.selected_clients is empty. Skipping.")
            return

        assert (len(self.selected_clients) > 0)

        # 确保 k 值不超过 self.selected_clients 的长度
        num_clients_to_sample = int((1 - self.client_drop_rate) * self.current_num_join_clients)
        num_clients_to_sample = min(num_clients_to_sample, len(self.selected_clients))  # 限制 k 的最大值

        if num_clients_to_sample <= 0:
            print("Warning: No clients to sample. Skipping.")
            return

        active_clients = random.sample(
            self.selected_clients, num_clients_to_sample)
    ...
    def aggregation_trimmed_mean(self, unlearning_stage=False, trimmed_clients_num=2, existing_clients=None):
        ...
        for channel_name in channels:
            channel_para = torch.zeros(self.uploaded_models[0].state_dict()[channel_name].size()).unsqueeze(0).to(
                self.device)
            for i in range(client_nums):
                channel_para = torch.cat(
                    (channel_para, self.uploaded_models[i].state_dict()[channel_name].unsqueeze(0)), dim=0)
    ...
    def load_client_model(self, epoch):
        ...
        print(f"Loaded client models: {len(client_models)}")
        print(f"Expected number of clients: {self.num_clients}")

        if len(client_models) > self.num_clients:
            print(f"Trimming client models from {len(client_models)} to {self.num_clients}")
            client_models = client_models[:self.num_clients]

        if len(client_models) < self.num_clients:
            print(f"Filling missing client models. Expected: {self.num_clients}, Loaded: {len(client_models)}")
            for _ in range(self.num_clients - len(client_models)):
                # 创建一个初始的 clientAVG 模型
                initial_model = clientAVG(self.args, id=-1, train_samples=0, test_samples=0, train_slow=False,
                                          send_slow=False)
                initial_model.set_parameters(copy.deepcopy(self.global_model))
                client_models.append(initial_model)

        assert (
                len(client_models) == self.num_clients), f"Mismatch in client models: {len(client_models)} != {self.num_clients}"
    ...
    def server_metrics(self):
        ...
        precision = precision_score(y_true, y_prob, average='macro')  # 使用macro平均
        recall = recall_score(y_true, y_prob, average='macro')
        f1 = f1_score(y_true, y_prob, average='macro')
        print("Average Test Accurancy: {:.4f}".format(acc))
        print("Average Test AUC: {:.4f}".format(auc))
        print("Average Test precision: {:.4f}".format(precision))
        print("Average Test recall: {:.4f}".format(recall))
        print("Average Test f1: {:.4f}".format(f1))
    ...
    def test_metrics(self):
        ...
        # 检查 num_samples 是否为空
        if not num_samples:
            print("Warning: No samples available for testing.")
            return [], [], [], []  # 返回空列表以避免后续计算错误
    ...
    def evaluate(self, acc=None, loss=None):
        ...
        if sum(stats[1]) == 0:
            print("Warning: No samples available for evaluation. Skipping evaluation.")
            return None, None  # 返回两个 None 值

        test_acc = sum(stats[2]) * 1.0 / sum(stats[1])
        test_auc = sum(stats[3]) * 1.0 / sum(stats[1])
        test_precision = sum(stats[4]) * 1.0 / sum(stats[1])
        test_recall = sum(stats[5]) * 1.0 / sum(stats[1])
        ...
        print("Averaged Test precision: {:.4f}".format(test_precision))
        print("Averaged Test recall: {:.4f}".format(test_recall))
    ...
    def MIA_attack(self, attacker, target_model, T=0.9):
        ...
        if target_model is None:
            print("target_model is None, please check its initialization.")
            return None, None  # 直接返回，避免后续代码执行

        target_model.eval()

        # 得到需要unlearn的client的 [logits, 1]
        unlearn_X = torch.zeros([1, N_class]).to(device)
    ...
    def retrain(self):
        ...
        for i in range(self.global_rounds + 1):
            ...
            if self.args.robust_aggregation_schemes == "FedAvg":
                ...
                self.aggregation_trimmed_mean(unlearning_stage=True,
