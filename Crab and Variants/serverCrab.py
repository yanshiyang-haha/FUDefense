...
class Crab(FedEraser):
    def __init__(self, args, times):
        ...
        self.P_rounds = 0.8
        self.X_clients = 0.8
        self.total_training_time = 0  # 新增：用于记录总训练时间
        self.total_recovery_time = 0  # 新增：用于记录总恢复时间
        # current_time = datetime.now().strftime("%Y%m%d-%H%M%S")

        # log_dir = f"../tf-logs/{current_time}_experiment"
        self.writer = SummaryWriter('../tf-logs/')
    ...
    def train_with_select(self):
        ...
        for i in range(self.global_rounds + 1):
            ...
            if i % self.eval_gap == 0:
                ...
                self.writer.add_scalar("FL_train_loss", train_loss, i)
                print("\n")

            if i == 0:
                start_loss = copy.copy(train_loss)
            else:
                GM_list.append(copy.copy(self.global_model))
            ...
            for client in self.selected_clients:
                if client in self.unlearn_clients and self.backdoor_attack:
                    ...
                    client.train(trim_attack=True, target_label=1)  # 假设目标类别为 1
                else:
                    client.train()
        ...
        self.total_training_time = training_end_time - training_start_time  # 计算总训练时间
        print("\nBest accuracy.")
        ...
        # self.server_metrics()
        self.FL_global_model = copy.copy(self.global_model)
        print("父类的train")
        print(f"Total training time: {self.total_training_time} seconds")  # 打印总训练时间

    def adaptive_recover(self):
        recovery_start_time = time.time()  # 记录恢复开始时间
        ...
        for global_round, select_clients_in_round in self.info_storage.items():
            ...
            self.old_CM = []
            added_client_ids = set()  # 用于存储已经添加到 old_CM 的客户端 ID
            ...
            for i, client in enumerate(self.old_clients):
                ...
                # 检查 client.id 是否在 select_clients_in_round 中，并且是否已经添加过
                if client.id in select_clients_in_round and client.id not in added_client_ids:
                    self.old_CM.append(client)  # 添加客户端到 old_CM
                    added_client_ids.add(client.id)  # 将客户端 ID 添加到集合中，避免重复
            print([c.id for c in self.old_CM])

            self.old_clients = copy.copy(self.old_CM)

            # 添加判断，确保 select_clients_in_round 不为空
            ...
            self.writer.add_scalar("recover_train_loss", train_loss, global_round)
        ...
        self.writer.close()
        recovery_end_time = time.time()  # 记录恢复结束时间
        self.total_recovery_time = recovery_end_time - recovery_start_time  # 计算总恢复时间
        print(f"\n-------------After Crab-------------")
        print("\nBest accuracy from unlearning.")
        print(best_unlearning_accuracy)
        print(f"Total recovery time: {self.total_recovery_time} seconds")  # 打印总恢复时间
        self.eraser_global_model = copy.copy(self.new_GM)
        self.new_CM = []